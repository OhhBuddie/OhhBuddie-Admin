           ...
           <i class="fa-solid fa-arrow-left"></i>
            ...
            <div class="collapse navbar-collapse" id="navbarContent">
                <div class="ms-auto d-flex navbar-actions">
                    ...
                    <a href="/seller/registration" class="btn btn-primary">
        ...
        <table id="sellerProductsTable" class="table table-hover table-bordered table-striped">
            <thead>
                <tr>
                    ...
                    <th>Total Brands</th>
                    <th>Total Product</th>
                    <th>Total Active Product</th>
                    <th>Total Inactive Product</th>
                    <th>Total Orders</th>
            ...
            <tbody>
                @foreach($seller_details as $sellers)
                
                   @php 
                                
                    $city_name = DB::table('cities')->where('id', $sellers->registered_city )->latest()->first();
                    $state_name = DB::table('states')->where('id', $sellers->registered_state )->latest()->first();
                    
                    $brnd_cnt = DB::table('brands')->where('seller_id',$sellers->seller_id)->count()
                    @endphp
       
                <tr>
                    <td>{{ $sellers->company_name }}</td> 
                    <td>{{ $sellers->registered_email_id }}</td>
                    <td>{{ $brnd_cnt }}</td>
                    <td>0</td>
                    <td>0</td>
                    <td>0</td> 
